...
def _impl_to_class_name(impl_name):
    """Convert a C++ impl string to a valid Python class name."""
    import re

    if re.match(r"^[A-Za-z_]\w*$", impl_name):
        return impl_name

    parts = [p for p in re.split(r"[^0-9A-Za-z_]+", impl_name) if p]
    class_name = "".join(p[:1].upper() + p[1:] for p in parts)
    if not class_name or not re.match(r"^[A-Za-z_]", class_name):
...
def generate_components(src_dir, python_dir, repo_root, dry_run=False):
    ...
    for impl_name, info in sorted(registrations.items()):
        ...
        class_name = _impl_to_class_name(impl_name)
