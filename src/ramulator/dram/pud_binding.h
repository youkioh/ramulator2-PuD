...
class PuDBinding {
 public:
  ...
  virtual int command(const DRAMSpec&, PuDCommand) const;
  virtual const std::vector<std::vector<TimingConsEntry>>& local_timing(const DRAMSpec&) const;
  ...
  virtual bool conventional_closed(const DRAMSpec&, const DRAMNode&) const;
  virtual bool conventional_drained(const DRAMSpec&, const DRAMNode&) const;
  virtual bool uses_bank_array(const DRAMSpec&, int command) const { return true; }
