...
- `visualizer/`
  Browser-based trace visualizer (Nuxt/WebGL). See [Section 10](#10-trace-visualizer).
...
- `BinTraceRecorder`
  Records a binary `.ram2bin` trace per channel for the [trace visualizer](#10-trace-visualizer)
- `LiveTraceStreamer`
  Streams commands to the [trace visualizer](#10-trace-visualizer) in real time over HTTP
...
Ramulator 2.1 includes a browser-based trace visualizer that renders command timelines, request swimlanes, and throughput charts. It supports two modes:

- **Offline** — Record a `.ram2bin` trace file, then open it in the browser.
- **Live** — Stream events from a running simulation to the browser in real time.

### 10.1 Docker

The visualizer is optional — `docker compose up` still only starts the Ramulator build container. Use Compose profiles to include it:

```bash
docker compose --profile visualizer up --build
```

This builds a self-contained production image and serves the visualizer at `http://localhost:3000`.
...
Override the port with `RAMULATOR_VISUALIZER_PORT`:
...
The visualizer must be built once before first use:
...
```bash
python -m ramulator visualize
```

This starts the production server and prints the URL (default `http://localhost:3000`). Options:
...
Add a `BinTraceRecorder` plugin to your controller to record a binary trace:

```python
controller_plugins=[
    ramulator.controller_plugin.BinTraceRecorder(path="my_trace"),
]
```

This produces one `.ram2bin` file per channel (e.g. `my_trace.ch0.ram2bin`). Open the visualizer in your browser and drag-and-drop the file onto the upload area.

### 10.4 Live Streaming

Add a `LiveTraceStreamer` plugin instead. Start the visualizer **first**, then run the simulation:

```bash
python -m ramulator visualize          # terminal 1
python my_config.py                    # terminal 2
```

```python
controller_plugins=[
    ramulator.controller_plugin.LiveTraceStreamer(
        port=3000,              # must match the visualizer port
        tick_interval=10000,    # flush every N simulator ticks
        update_interval_s=0.5,  # or every N wall-clock seconds
    ),
]
```
